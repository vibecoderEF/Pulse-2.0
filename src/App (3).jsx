...
function App() {
  ...
  // On mount: push local data up, then pull remote and merge if newer
  useEffect(() => {
    if (!SYNC_AVAILABLE) return;
    (async () => {
      setSyncStatus("syncing");

      // 1. Always push current local state so phone data reaches Supabase
      const localRaw = localStorage.getItem("pulseV1");
      const localState = localRaw ? JSON.parse(localRaw) : null;
      const isFreshDevice = !localRaw || !localState?._syncedAt;
      if (localState && !isFreshDevice) {
        // Phone/existing device: push up immediately
        await syncPush({ ...localState, _syncedAt: localState._syncedAt || Date.now() });
      }

      // 2. Pull remote row
      const row = await syncPull();
      if (row?.data) {
        const remote = migrateState(row.data);
        const localTs  = localState?._syncedAt || 0;
        const remoteTs = row.data._syncedAt || new Date(row.updated_at).getTime();

        // Fresh device (no local data) OR remote is newer → apply remote
        if (isFreshDevice || remoteTs > localTs) {
          const merged = { ...remote, _syncedAt: remoteTs };
          DR(merged);
          saveState(merged, () => setStorageErr(true));
          toast("✓ Data synced from cloud");
        }
      } else if (isFreshDevice) {
        // No remote data found at all — token may be wrong
        setSyncStatus("error");
        return;
      }
  ...
  const D = useCallback(action => {
    DR(prev => {
      ...
      // Debounced push — fires 800 ms after last action
      if (SYNC_AVAILABLE) {
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          {SYNC_AVAILABLE && (
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              ...
              {/* Share / copy link button — also triggers an immediate push */}
              <button
                title="Copy sync link"
                onClick={async () => {
                  // Force push current state immediately before sharing
                  setSyncStatus("syncing");
                  const localRaw = localStorage.getItem("pulseV1");
                  if (localRaw) {
                    const localState = JSON.parse(localRaw);
                    const stamped = { ...localState, _syncedAt: Date.now() };
                    saveState(stamped, () => {});
                    DR(stamped);
                    await syncPush(stamped);
                  }
                  setSyncStatus("ok");
                  setTimeout(() => setSyncStatus(s => s==="ok" ? "idle" : s), 3000);
                  ...
                  try {
                    ...
                    toast("Data uploaded & link copied! Open on any device 🔗");
                  } catch {
                    toast(url.toString());
                  }
                }}
